...
#define SGL_SCALAR_TYPE_NAME(type) torch::headeronly::toString(type)
...
#define SGL_SCALAR_TYPE_NAME(type) type
...
#define CHECK_TYPE(x, st)                    \
  SGL_CHECK(                                 \
      x.scalar_type() == st,                 \
      #x " dtype is ",                       \
      SGL_SCALAR_TYPE_NAME(x.scalar_type()), \
      ", while ",                            \
      SGL_SCALAR_TYPE_NAME(st),              \
      " is expected")
#define CHECK_TH_CUDA(x) SGL_CHECK(x.is_cuda(), #x " must be a CUDA tensor")
#define CHECK_CONTIGUOUS(x) SGL_CHECK(x.is_contiguous(), #x " must be contiguous")
...
void launchFusedQKNormRope(
  ...
  switch (head_dim) {
    ...
    case 256:
      ...
      break;
    default:
      SGL_CHECK(false, "Unsupported head dimension for fusedQKNormRope: ", head_dim);
  }
}
}  // namespace tensorrt_llm::kernels
...
void fused_qk_norm_rope(
    Tensor& qkv,           // Combined QKV tensor [num_tokens, (num_heads_q+num_heads_k+num_heads_v)*head_dim]
    int64_t num_heads_q,   // Number of query heads
    int64_t num_heads_k,   // Number of key heads
    int64_t num_heads_v,   // Number of value heads
    int64_t head_dim,      // Dimension per head
    double eps,            // Epsilon for RMS normalization
    Tensor& q_weight,      // RMSNorm weights for query [head_dim]
    Tensor& k_weight,      // RMSNorm weights for key [head_dim]
    double base,           // Base for RoPE computation
    bool is_neox,          // Whether RoPE is applied in Neox style
    Tensor& position_ids,  // Position IDs for RoPE [num_tokens]
    // parameters for yarn
    ...
    int64_t rotary_dim) {
  // Input validation
  SGL_CHECK(qkv.dim() == 2, "QKV tensor must be 2D: [num_tokens, (num_heads_q+num_heads_k+num_heads_v)*head_dim]");
  SGL_CHECK(position_ids.dim() == 1, "Position IDs must be 1D: [num_tokens]");
  SGL_CHECK(q_weight.dim() == 1, "Query weights must be 1D: [head_dim]");
  SGL_CHECK(k_weight.dim() == 1, "Key weights must be 1D: [head_dim]");
  SGL_CHECK(q_weight.size(0) == head_dim, "Query weights size must match head dimension");
  SGL_CHECK(k_weight.size(0) == head_dim, "Key weights size must match head dimension");
  SGL_CHECK(rotary_dim % (head_dim / 32) == 0, "rotary_dim must be divisible by numElemsPerThread");
  if (is_neox) {
  ...
  CHECK_INPUT(qkv, ScalarType::BFloat16);
  CHECK_INPUT(position_ids, ScalarType::Int);
  CHECK_INPUT(q_weight, ScalarType::BFloat16);
  CHECK_INPUT(k_weight, ScalarType::BFloat16);

  int64_t num_tokens = qkv.size(0);
  SGL_CHECK(position_ids.size(0) == num_tokens, "Number of tokens in position_ids must match QKV");

  int64_t total_heads = num_heads_q + num_heads_k + num_heads_v;
  SGL_CHECK(
      qkv.size(1) == total_heads * head_dim, "QKV tensor size must match total number of heads and head dimension");

  auto stream = SGL_CURRENT_CUDA_STREAM(qkv);

  tensorrt_llm::kernels::launchFusedQKNormRope(
      reinterpret_cast<__nv_bfloat16*>(SGL_MUTABLE_DATA_PTR(qkv, BFloat16)),
      ...
      reinterpret_cast<const __nv_bfloat16*>(SGL_CONST_DATA_PTR(q_weight, BFloat16)),
      reinterpret_cast<const __nv_bfloat16*>(SGL_CONST_DATA_PTR(k_weight, BFloat16)),
      static_cast<float>(base),
      !is_neox,  // interleave
      SGL_CONST_DATA_PTR(position_ids, int32_t),
