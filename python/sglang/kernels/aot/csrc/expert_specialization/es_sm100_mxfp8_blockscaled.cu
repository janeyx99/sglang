...
void es_sm100_mxfp8_blockscaled_grouped_mm(
  ...
  STD_TORCH_CHECK(a.dim() == 2, "a must be a 2D tensor of shape (num_tokens, k)");
  STD_TORCH_CHECK(b.dim() == 3, "b must be a 3D tensor of shape (num_experts, k, n)");
  STD_TORCH_CHECK(a.size(1) == b.size(1) && a.size(1) % 128 == 0, "k should align 128");
  STD_TORCH_CHECK(b.size(2) % 128 == 0, "n should align 128");
  STD_TORCH_CHECK(a.strides()[1] == 1, "a must be row major");
  STD_TORCH_CHECK(b.strides()[1] == 1, "a must be column major");
